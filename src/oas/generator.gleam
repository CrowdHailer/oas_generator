...
pub fn build(spec_src, project_path, provider, exclude) {
  ...
  use file <- try(
    ...
    |> snag.map_error(fn(_) { "Unable to read " <> spec_src }),
  )
  ...
  use Nil <- try(
    ...
    |> snag.map_error(fn(_) { "Unable to write to " <> operations_module_file }),
  )

  use mod <- try(
    simplifile.read(module_path <> ".gleam")
    |> snag.map_error(fn(_) { "Unable to read " <> module_path <> ".gleam" }),
  )
  ...
  use Nil <- try(
    ...
    |> snag.map_error(fn(_) { "Unable to write to " <> module_path <> ".gleam" }),
  )
  ...
  use Nil <- try(
    ...
    |> snag.map_error(fn(_) { "Unable to write to " <> schema_module_file }),
  )
  Ok(Nil)
}
